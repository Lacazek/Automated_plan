...
namespace DoseCheck
{
    internal class GetMyData : INotifyPropertyChanged
    {
        ...
        internal GetMyData(UserInterfaceModel _m)
        {
        ...
        public void MyData()
        {
            ...
            int i = 1;
            int addNLines = 0;
            PlanSetup myPlan = _model.GetContext.PlanSetup;
            ...
            try
            {
                foreach (var index in _model.GetContext.PlanSetup.RTPrescription.Targets)
                {
                ...
                foreach (var OAR in _model.GetContext.PlanSetup.RTPrescription.OrgansAtRisk)
                {
            ...
            catch (Exception ex)
            {
                ...
                _model.Set_Log = $"Erreur sur la prescription : aucune prescription n'est rattachée \n";
            }
            #endregion
            ...
            if (_model.GetContext.ExternalPlanSetup.IsDoseValid)
            {
                while ((line = _streamReader.ReadLine()) != null)
                {
                    ...
                    try
                    {
                        NbFraction = _model.GetContext.PlanSetup.RTPrescription.Targets.FirstOrDefault(x => x.TargetId == st.Id).NumberOfFractions;
                        DosePerFraction = _model.GetContext.PlanSetup.RTPrescription.Targets.FirstOrDefault(x => x.TargetId == st.Id).DosePerFraction;
                    }
                    catch
                    {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _model.Set_Log = $"Erreur sur le calcul des doses : La structure {st.Id} a été trouvé à la place de {line.Split(';')[0]} \n";
                    }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _model.Set_Log = $"Problème dans la récupération des doses pour {_s} (exemple : Vxx% ou Dxx%\n";
                    }
                    #endregion
                    ...
                    if (_token)
                    {
                        foreach (var _target in myPlan.StructureSet.Structures)
                        {
                            ...
                            catch (Exception ex)
                            {
                                ...
                                _model.Set_Log = $"Erreur dans le calcul des indices stéréotaxiques\n";
                            }
                ...
                _model.Set_Log = $"La dose n'est pas valide (le plan n'est peut être pas calculé)\n";
        ...
        internal string UserFileCheck
        {
            ...
            set
            {
                _userFile = System.IO.Path.Combine(System.IO.Path.Combine(_model.GetCheckPath, "Template_dosi"), value + ".txt");
                MyData();
            }
        }
